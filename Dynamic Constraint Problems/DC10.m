    ...
    methods
        ...
        function PreGenerateOptimumsAndFeasibleRegions(obj)
            ...
            for i = 1:length(t)
                ...
                %% 
                z = nan(size(x));
                c11 = (x).^1.5+ (y).^1.5 - 1.2^1.5;
                c12 = x.^(0.5) + y.^(0.5) - (0.95+0.5*abs(G) );
                c21 =  0.8*x + y - (2.5 + 0.08*sin(2*pi*(y-x)));
                c22 = (0.93 + abs(G)/3)*x + y - (2.7+abs(G)/2 + 0.08*sin(2*pi*(y-x)));
                c1  = c11.*c12;
                c2  = c21.*c22;
