    ...
    methods
        ...
        function PreGenerateOptimumsAndFeasibleRegions(obj)
            ...
            for i = 1:length(t)
                ...
                pf=[];  
                pf=pf1;
                G = 2*floor(10*abs(mod(ti+1,2)-1)+1e-4) ; 
                c12 = (pf(:,1)).^2 + (pf(:,2)).^2 - (1.5 + 0.4*sin(4*atan(pf(:,2)./pf(:,1))).^16).^2;
                c11 = (pf(:,1)).^2 + (pf(:,2)).^2 - (1.3 - 0.45*sin(G*atan(pf(:,2)./pf(:,1))).^2).^2;
                con=-c11.*c12;
                pf(con>0,:)=[];
                pf=sortrows(pf);
                ...
                [x, y] = meshgrid(linspace(0, 2, 500), linspace(0, 2, 500));
                %% 
                z = nan(size(x));

                c12 = (x).^2 + (y).^2 - (1.5 + 0.4*sin(4*atan(y./x)).^16).^2;
                c11 = (x).^2 + (y).^2 - (1.3 - 0.45*sin(G*atan(y./x)).^2).^2;
                con=-c11.*c12;
        ...
        function PopObj = CalObj(obj, PopDec)
            ...
            temp=(PopDec(:,2:end)-0.5);
            g=1+sum( (temp.^2 -cos(pi.*temp) + 1).^2,2);
        ...
        function PopCon = CalCon(obj, PopDec, PopObj)
            ...
            t = obj.t;
            G = 2*floor(10*abs(mod(t+1,2)-1)+1e-4) ; 
            c12 = (PopObj(:,1)).^2 + (PopObj(:,2)).^2 - (1.5 + 0.4*sin(4*atan(PopObj(:,2)./PopObj(:,1))).^16).^2;
            c11 = (PopObj(:,1)).^2 + (PopObj(:,2)).^2 - (1.3 - 0.45*sin(G*atan(PopObj(:,2)./PopObj(:,1))).^2).^2;
            PopCon=-c11.*c12;
        end
